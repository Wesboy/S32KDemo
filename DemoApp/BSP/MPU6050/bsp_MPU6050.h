...
#define PWR_MGMT_1 0x6B // Device defaults to the SLEEP mode//电源管理，典型值：0x00(正常启用)
...
#define WHO_AM_I_MPU6050 0x75 // Should return 0x68//IIC地址寄存器(默认数值0x68，只读)
...
uint32_t Single_ReadI2C(uint8_t *pBuffer, uint8_t ReadAddr); //读取I2C数据
uint32_t Single_WriteI2C(uint8_t WriteAddr, uint8_t iCmd);   //向I2C写入数据
